...
class GabCaptcha2_Options
{
	...
	public function gabcaptcha2_options_page()
	{
		...
		<p>' . __( 'Translated by <a href="http://www.gabsoftware.com/">Gabriel Hautclocq</a>', 'gabcaptcha2' ) . '</p>'
		;
	}
	...
	public function gabcaptcha2_options_validate_callback($input)
	{
		...
		{
			$newinput['automatically_approve'] = $input['automatically_approve'];
	...
	public function display_setting_callback( $args = array() )
	{
		...
		switch ( $type )
		{
			...
			case 'textarea':
				...
				echo "<textarea{$field_class} id='{$id}' name='gabcaptcha2_options[{$id}]' placeholder='{$std}'>{$value}</textarea>";
			...
			case 'password':
				$value = $options[$id];
				echo "<input{$field_class} type='password' id='{$id}' name='gabcaptcha2_options[{$id}]' value='{$value}' />";
			...
			default:
				...
				$value = ( isset( $options[$id] ) ? $options[$id] : $std );
				echo "<input{$field_class} type='text' id='{$id}' name='gabcaptcha2_options[{$id}]' placeholder='{$std}' value='{$value}'{$sizeattribute} />";
				if ( $desc != '' )
				{
					echo "<br /><small>{$desc}</small>";
				}
	...
	} //function display_setting_callback
} //class GabCaptcha2_Options
